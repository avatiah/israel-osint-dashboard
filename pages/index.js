// [Вставь этот код полностью, он оптимизирован под Твои требования]
import React, { useEffect, useState } from 'react';

const highlightCritical = (text) => {
  const words = ["ВЫСОКАЯ", "аномальная", "ДЕЖУРСТВО", "усиленного", "ПРЕДУПРЕЖДЕНИЕ", "БПЛА", "обход", "удар"];
  let formatted = text;
  words.forEach(word => {
...
export default function TerminalV15() {
  ...
  useEffect(() => {
    updateData();
    const timer = setInterval(updateData, 15000); // Оптимальный интервал для динамики
    return () => clearInterval(timer);
  }, []);

  if (!data) return <div style={s.loader}>{">"} СИНХРОНИЗАЦИЯ С OSINT-УЗЛАМИ...</div>;
  ...
  return (
    <div style={{
      ...s.container,
      border: isNotamActive ? '1px solid #900' : '1px solid #111',
      ...
      <header style={s.header}>
        ...
        <div style={s.meta}>V15.0 // МОНИТОРИНГ_В_РЕАЛЬНОМ_ВРЕМЕНИ // ОБНОВЛЕНО: {lastUpdate}</div>
      </header>
      ...
      <main style={s.grid}>
          ...
          <div key={node.id} style={s.card}>
            ...
            <div style={s.infoBox}>
              <div style={s.metricsList}>
                ...
                <span style={s.metricItem}>ТРАФИК ИРАНА: <b style={{color:'#fff'}}>{data.netConnectivity.score}%</b></span>
                <span style={s.metricItem}>DIPLOMACY: <b style={{color:'#888'}}>CRITICAL_STALEMATE</b></span>
        ...
        <div style={s.forecastBox}>
          <div style={s.forecastTitle}>⚠️ ПРОГНОЗ: {data.prediction.date}</div>
          <div style={s.forecastText}>ВЕРОЯТНОСТЬ ЭСКАЛАЦИИ ПРИ ОТСУТСТВИИ СДЕЛКИ: <strong>{data.prediction.impact}%</strong></div>
        </div>
      </main>
      ...
      <footer style={s.footer}>
        ...
        <div style={s.footerMeta}>MADAD HAOREF // ТЕРМИНАЛ_2026</div>
      </footer>
    </div>
  );
}
...
const s = {
  ...
  apiIndicator: { position: 'absolute', top: '20px', right: '20px', display: 'flex', alignItems: 'center', gap: '8px' },
  ...
  card: { border: '1px solid #111', padding: '20px', background: '#050505' },
  cardLayout: { display: 'flex', gap: '20px', alignItems: 'center', flexWrap: 'wrap' },
  cardContent: { flex: 1 },
  nodeTitle: { fontSize: '12px', color: '#fff', fontWeight: 'bold', marginBottom: '12px', borderBottom: '1px solid #111', paddingBottom: '4px' },
  ...
  infoBox: { borderTop: '1px solid #111', marginTop: '15px', paddingTop: '15px' },
  metricsList: { display: 'flex', gap: '15px', flexWrap: 'wrap' },
  infoLabel: { fontSize: '9px', color: '#0f4' },
  metricItem: { fontSize: '9px', color: '#444' },
  forecastBox: { border: '1px solid #400', padding: '20px', background: '#0a0000', textAlign: 'center' },
  forecastTitle: { fontSize: '12px', color: '#f33', marginBottom: '8px' },
  forecastText: { fontSize: '11px', color: '#fff' },
  footer: { marginTop: '60px', textAlign: 'center', borderTop: '1px solid #111', paddingTop: '20px' },
  disclaimer: { fontSize: '8px', color: '#444', maxWidth: '500px', margin: '0 auto' },
  footerMeta: { fontSize: '8px', color: '#030', marginTop: '10px' },
