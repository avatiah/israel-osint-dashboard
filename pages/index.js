...
const Gauge = ({ value, label, status, range }) => (
  ...
  return (
    <div className="admin-container">
      <header className="header">
        <div className="title">THREAT_ENGINE_ADMIN // V65.STABLE</div>
        <div className="clock">{new Date(data.updated).toLocaleTimeString()}</div>
      </header>

      <div className="main-grid">
        <section className="panel">
          <div className="p-title">INTERNAL_EXTERNAL_THREAT_GAUGES</div>
      ...
      <footer className="footer">
        THREAT_LEVEL_MONITOR // PRO_EDITION // 2026_COMPLIANT
      </footer>
      ...
      <style jsx global>{`
        ...
        .header { display: flex; justify-content: space-between; border-bottom: 2px solid #0f0; padding-bottom: 10px; margin-bottom: 20px; font-weight: bold; }
        ...
        .g-lab { font-size: 0.6rem; text-align: center; color: #060; }
        .m-table { width: 100%; border-collapse: collapse; }
        .m-table td { border: 1px solid #040; padding: 8px; font-size: 1.1rem; }
        .sig-list { font-size: 0.8rem; line-height: 1.6; }
        .sig-on { color: #0f0; } .sig-off { color: #030; }
        .feed-scroll { height: 100px; overflow-y: auto; font-size: 0.8rem; border-top: 1px solid #040; padding-top: 10px; }
        .f-line { margin-bottom: 5px; color: #aaa; }
        .green { color: #0f0; } .loading { text-align: center; padding: 50px; font-size: 1.5rem; }
