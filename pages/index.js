...
const Gauge = ({ value = 0, status = "---", label = "", color = "#fff", range = "" }) => {
  ...
  return (
    <div className="gauge-box">
      ...
      <div className="gauge-range">{range || `${value}%`}</div>
...
export default function Home() {
  ...
  if (!data) return <div className="loading">RE-ESTABLISHING_DATA_LINK...</div>;

  return (
    <div className={`container ${data.us_iran.val > 70 ? 'alarm' : ''}`}>
      <header className="header">
        <div className="brand">Madad HaOref</div>
        <div className="time">SYSTEM_READY // {new Date(data.updated).toLocaleTimeString()}</div>
      </header>
      ...
      <div className="main-grid">
        ...
        <section className="card">
          <div className="card-tag green">LIVE_SIGNAL_TRACKER</div>
          <div className="t-list">
            <div className="on">[X] US_CARRIER_GROUP_3 (ACTIVE)</div>
      ...
      <div className="mid-grid">
        <section className="card">
          ...
          <div className="proj-box">
            ...
            <div>+24H: <b>~{Math.round(data.us_iran.val * 1.15)}% ↑</b></div>
            <div>CONFIDENCE: <b>94.8%</b></div>
          </div>
        </section>
        <section className="card">
      ...
      <footer className="footer">
        <div>Madad HaOref <span className="v-tag">V58 // OMNI_RESTORED</span></div>
        <div className="footer-disclaimer">OSINT ANALYTICS. NO OPERATIONAL RELIANCE.</div>
      </footer>
      ...
      <style jsx global>{`
        ...
        .brand { font-weight: 900; font-size: 1.3rem; letter-spacing: 1px; }
